<template>
  <div class="w-full container mx-auto my-10">
    ...
    <div class="w-full flex md:flex-row xs:flex-col">
      <!-- <div class="w-3/4 flex"> -->


      <div class="xs:w-full sm:w-full md:w-3/5 lg:w-3/5 lg:mr-14 md:mr-14 xs:mr-0 sm:mr-0 bg-white  md:p-10 xs:p-0 shadow">
        <p class="text-red-900 mr-5" v-if="errors.shipping_method_id">
          {{ errors.shipping_method_id[0] }}
        </p>
        

        <div class="lg:w-3/5 lg:mr-16 md:mr-16 xs:mr-0 sm:mr-0 bg-white lg:px-6 md:px-6 xs:pr-6	sm:pr-6 py-3 shadow">
          <ShippingAddress v-model="form.address_id" />

        </div>
      </div>


      <div class="xs:w-full sm:w-full md:w-2/5 lg:w-2/5">
        <div class="bg-white shadow p-4">
          <p class="text-center text-xl border-b border-gray-500 pb-2  mb-4">Price Details</p>
          ...
          <div class="flex justify-between my-4">
            ...
            <p class="font-semibold text-shop-primary" style="color: #f02640;">{{ total }}</p>
          </div>
          <!--  <div class="flex justify-between border-b border-gray-500 pb-2 my-4">
                <p>Sub Total</p>
                <p>{{subtotal}}</p>
              </div>
    ...
    <div class="border-gray-500 ">
      <p class="text-red-900" v-if="errors.payment_method_id">
        {{ errors.payment_method_id[0] }}
      </p>
      <div class="w-full flex justify-center">
        <PaymentMethods :order_id="orderid" v-model="form.payment_method_id" />

  
      </div>
      <article class="message">
        <div class="message-body flex justify-center">
          <button
            class="button is-info is-fullwidth is-medium no-underline w-48 no-underline secondary hover:bg-black p-3 text-white rounded-full font-semibold outline-none cbc"
            :disabled="empty || submitting" v-show="btnshow" @click.prevent="order">
            Place order
          </button>
        </div>
      </article>
