		...
		<div class="min-w-screen min-h-screen bg-gray-300 flex items-center justify-center px-5 py-16">
			<div class="bg-gray-100 text-gray-500 rounded-3xl shadow-xl w-full overflow-hidden" style="max-width:1000px">
				<div class="md:flex w-full">
					...
					<div class="w-full md:w-1/2 py-16 px-5 md:px-10 mx-auto">
						<!-- <div class="w-full md:w-1/2 py-5 px-5 md:px-10 h-[850px] mx-auto flex justify-center flex-col"> -->
						<div class="text-center mb-10">
							<h1 class="font-bold text-3xl text-gray-900">Forgot Your Password?</h1>
						...
						<div v-if="failure != null" class="w-full my-4 bg-red-500 p-4 flex justify-between rounded-lg"
							id="success-alert">
							...
							<form action="" @submit.prevent="store">
								...
								<div class="flex mx-3">
									<div class="w-full px-3 my-5 ">
										<button
											class="block w-full max-w-xs mx-auto secondary hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-full px-3 py-3 font-semibold">Submit</button>
									</div>
								</div>
							</form>
...
export default {
	...
	methods: {
		...
		async store() {
			this.show = true;
			let response = await this.$axios.$post('auth/userresetpassword', this.form).then((response) => {
				if (response.message !== "Email send successfully.Please check your email.") {
