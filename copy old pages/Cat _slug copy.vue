    ...
    <div class="container mx-auto w-full flex justify-center  mb-10">
      <div class="w-full flex">
        <div class="w-1/6 flex mr-1 flex-col">
          <div class="bg-white w-full">
            ...
            <ul class="list-reset ml-3 p-2" v-for="(seller, index) in sellers">
              <li class="text-xs justify-between flex"><a href="#" @click="searchBrand(seller.id)"
                  class="cursor-pointer no-underline text-black hover:text-grey">{{ seller.name }}</a><span></span></li>
        ...
        <div class="w-5/6 flex flex-wrap">
          <div v-if="products.length > 0" class="w-1/4" v-for="(product, index) in products" :key="product.id">
            <div class="port mb-2 p-2 bg-white mr-2">
              <div class="flex flex-col">
                <div class="flex justify-end">
                  <div class="portfolioDisc absolute flex flex-col p-2">
                    <a href='#' class="text-center no-underline text-white p-2 bg-gray-500 my-1 hover:text-grey"
                      title="wishlist">
                ...
                <div v-if="product.productgallery.length > 0">
                  <nuxt-link :to="{ name: 'products-slug', params: { slug: product.slug } }">
                    <img v-bind:src="rooturl + product.productgallery[0].imagepath"
                      style="width:250px;height: 250px"></nuxt-link>
                </div>
                ...
                <div class="w-5/6 mr-1">
                  ...
                  <div class="flex px-2 py-4">
                    <div v-for="rating in total_rating">
                      ...
                      <div v-else>
                        <img src="/images/star.svg" class="h-3">
                      </div>
                    </div>
                  </div>
                </div>
                <div class="w-1/6 portfolioDisc">
                  <p @click="productredirect(product)"
                    class="p-2 secondary text-white text-center rounded hover:bg-black font-semibold"><img
                      src="/images/cart.svg" class="h-4"></p>
                </div>
              </div>
            </div>
          </div>
