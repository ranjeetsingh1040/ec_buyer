        ...
        <div class="navbar-end flex float-right lg:mr-5">
          ...
          <template v-else>
            <div class="flex items-center">
              ...
              <nuxt-link :to="{ name: '' }" class="navbar-item rounded-full"><img :src="rooturl + $auth.user.image" class="h-8"
                  @click="togglemenu">
              </nuxt-link>
            </div>
          </template>
      ...
      <template v-if="$auth.loggedIn">
        ...
        <div class="w-64 sub-menu flex items-start mx-4">
          <div class="w-full p-2 flex flex-col">
            <p class="text-sm font-bold uppercase text-center text-white border-b border-gray-500 pb-2">{{$auth.user.name}}</p>
