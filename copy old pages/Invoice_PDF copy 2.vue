<template>
  <div>
    ...
    <div v-for="invoicedetail in invoicedetails" style="width:100%;display:grid;margin-left: 33%;" class="invoice p-3 mb-3" id="mydiv">
      <div style="display:inline-flex;margin-bottom:.75rem">
        <div style="width:10%;">
          <nuxt-link :to="{ name: 'index' }" class="text-white nlink">
            <img src="/logo.png" class="h-16" style="max-width:80px;height:27px">
          </nuxt-link>
        </div>
        <div style="width:22%;text-align: right">
          <p style="font-size:10px;"><span style="font-weight: 700">Tax Invoice/Bill of Supply/Cash Memo</span>
            <br />(Orginial for Recipient)</p>
        </div>
      </div>
      <div style="width:32%;display:flex;justify-content:space-between;;margin-bottom:.75rem">
        <div>
          <p style="font-size:10px;">Sold By:{{ invoicedetail.stores.sellername }}<br></p>
          <p style="font-size:10px;">{{ invoicedetail.stores.selleraddress }}<br></p>
        </div>
        <div style="text-align: right;">
          ...
          <p style="font-size:10px;">{{ invoicedetail.address.address_1 }}
            <!-- ,<br>{{invoicedetail.address.address_2}} -->
          </p>
          <p style="font-size:10px;">{{ invoicedetail.address.city.name }}<br></p>
          <p style="font-size:10px;">{{ invoicedetail.address.state.name }}-{{ invoicedetail.address.postal_code }}<br></p>
      ...
      <div style="width:32%;display:flex;justify-content:flex-end;margin-bottom:.75rem">
        <div style="text-align: right;">
          ...
          <p style="font-size:10px;">{{ invoicedetail.address.firstname }} {{ invoicedetail.address.lastname }}<br></p>
          <p style="font-size:10px;">{{ invoicedetail.address.address_1 }}
            <!-- ,<br>{{invoicedetail.address.address_2}}<br> -->
          </p>
          <p style="font-size:10px;">{{ invoicedetail.address.city.name }}<br></p>
          <p style="font-size:10px;">{{ invoicedetail.address.state.name }}-{{ invoicedetail.address.postal_code }}<br></p>
          <p style="font-size:10px;">{{ invoicedetail.address.country.code }}<br></p>
      ...
      <div style="width:32%;display:flex;justify-content:space-between;margin-bottom:.75rem">
        <div>
          ...
          <div v-if="invoicedetail.transactions.length > 0">
            <p style="font-size:10px;text-align: right;" v-if="invoicedetail.payment_method.card_type == 'PayTM'"> PAYTM
              MAILID :
      ...
      <div style="width:32%;display:flex;justify-content:space-between;padding:5px;margin-bottom:.5rem"
        ...
        <div style="width:8%;">
          <p style="font-size:7px;text-align: right;">{{ Number(productlist.shippingtaxprice) + Number(productlist.itemtax) }}
          </p>
        </div>
        <div style="width:8%;">
